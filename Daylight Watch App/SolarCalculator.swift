...
struct SolarCalculator {
    ...
    public init(forLocation location: CLLocation, atDate date: Date){
        ...
        if (date < sun.decemberSolstice && date < sun.juneSolstice) {
            ...
            sunAtSolstice = Sun(location: location, timeZone: Calendar.current.timeZone, date: last_sun.decemberSolstice);
        } else if (date < sun.decemberSolstice) {
            sunAtSolstice = Sun(location: location, timeZone: Calendar.current.timeZone, date: sun.juneSolstice);
        } else {
            sunAtSolstice = Sun(location: location, timeZone: Calendar.current.timeZone, date: sun.decemberSolstice);
        }
    }
    
    public var pastSolstice: Date {
        get { sunAtSolstice.date }
    }
    ...
    public var morningTimeSinceSolistice: TimeInterval {
        get {
            ...
            let seconds_now = cal.component(.hour, from: now) * 60 * 60
            + cal.component(.minute, from: now) * 60
            + cal.component(.second, from: now)
            
            let seconds_then = cal.component(.hour, from: then) * 60 * 60
